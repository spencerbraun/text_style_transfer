...
def init_params(options):
    ...
    params["style_emb"] = norm_weight(options['senti_num'],options["senti_dim"])
    ctxdim = ctxdim + options["senti_dim"]
...
def build_model(tparams, options):
    ...
    se = tparams["style_emb"]
    se  = se[si.flatten()]
    ctx = concatenate([se, ctx], axis=1)
    ...
    return trng, use_noise, x, x_mask, y, y_mask, si, opt_ret, cost, cost_d, cost_h
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    outs = [init_state, ctx]
    f_init = theano.function([x, si], outs, name='f_init', profile=profile)
    print 'Done'
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=False):
    ...
    for x, y, senti in iterator:
        ...
        pprobs = f_log_probs(x, x_mask, y, y_mask, senti)
        for pp in pprobs:
            probs.append(pp)
...
def train(dim_word=100,  # word vector dimensionality
    ...
    grads_o = tensor.grad(cost, wrt=itemlist(tparams), disconnected_inputs='ignore')
    ...
    i = 0
    for kk,vv in tparams.iteritems():
        if "style_matrix" in kk and style_class:
            grads.append(grads_o[i]+grads_d[i]*weight_d)
        elif "encoder" in kk and style_adv:
            grads.append(grads_o[i]+grads_h[i]*weight_h)
        else:
            grads.append(grads_o[i])
        i += 1
    ...
    for kk,vv in tparams.iteritems():
        if "style_matrix" in kk and style_class:
            grads_noadv.append(grads_o[i]+grads_d[i]*weight_d)
        else:
            grads_noadv.append(grads_o[i])
        i += 1
    ...
    f_grad_shared, f_update = eval(optimizer)(lr, tparams, grads, inps, cost, cost_d, cost_h, "1")
    f_grad_shared_noadv, f_update_noadv = eval(optimizer)(lr, tparams, grads_noadv, inps, cost, cost_d, cost_h, "2")
    print 'Done'

    print 'Building style embedding distance',
    s_e = tparams["style_emb"]
    s_e = s_e[0]-s_e[1]
    s_e = s_e * s_e
    s_e = s_e.sum()
    style_distance = theano.function([],s_e,profile=profile)
    print 'Done'
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y, senti in train:
            ...
            if use_adv:
                ...
                cost, cost_d, cost_h = f_grad_shared(x, x_mask, y, y_mask, senti)
                # do the update on parameters
                f_update(lrate)
            else:
                # compute cost, grads and copy grads to shared variables
                cost, cost_d, cost_h = f_grad_shared_noadv(x, x_mask, y, y_mask, senti)
                # do the update on parameters
                f_update_noadv(lrate)
            ...
            if numpy.mod(uidx, dispFreq) == 0:
                print 'Epoch ', eidx, 'Update ', uidx, 'Cost ', cost, cost_d, cost_h, 'style_dis', style_dis, 'UD ', ud
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                for jj in xrange(numpy.minimum(5, x.shape[1])):
                    ...
                    sample1, score1 = gen_sample(tparams, f_init, f_next,
    ...
    if params is None: params = tparams
    pkl.dump(model_options, open('%s.pkl' % saveto, 'wb')) 
